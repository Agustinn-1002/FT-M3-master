...
async function problemA () {
 ...
 function problemB () {
  ...
  async function rearFileAsync(text){
    blue(await promisifiedReadFile(text))
  }
  rearFileAsync('poem-one/stanza-02.txt')
  rearFileAsync('poem-one/stanza-03.txt')
...
async function problemC () {
  ...
  await rearFileAsync('poem-one/stanza-02.txt')
  await rearFileAsync('poem-one/stanza-03.txt')
  console.log('done'); 
}
...
async function problemE () {
  ...
  try {
    ...
    const stanza2 = await promisifiedReadFile('poem-one/wrong-file-name.txt');
    blue(stanza2)
  } catch (error) {
    magenta(error);
  }
