...
function problemC () {
  ...
  const allData = ['poem-one/stanza-02.txt','poem-one/stanza-03.txt'];
  const data = allData.map(fD => promisifiedReadFile(fD));

  Promise.all(data)
    .then(stanza3 => {
      stanza3.map(key => blue(key))
...
function problemE () {
  ...
  // promise version
  const allData = ['poem-one/stanza-03.txt','poem-one/wrong-file-name.txt'];
  const data = allData.map(fD => promisifiedReadFile(fD));
...
function problemF () {
  ...
  Promise.all(data)
    ...
    .finally(() => console.log('-- F. Promise version done --')) // SE EJECUTA SIEMPRE Y NO LE DA BOLA SI DIO O NO LA PROMESA 
